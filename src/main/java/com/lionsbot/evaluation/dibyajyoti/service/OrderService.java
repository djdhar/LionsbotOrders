...
public class OrderService {
    ...
    public List<Order> getOrdersByCustomerId(UUID customerId, String token) {
        if(jwtUtil.isAdminToken(token) ||
    ...
    public Order addOrder(Order order, String token) {
        ...
        order.setCustomerId(UUID.fromString(jwtUtil.extractUsername(token.substring(7))));
        return orderRepository.save(order);
    }

    public Order modifyOrder(UUID orderId, Order order, String token) {
    ...
    public String deleteOrder(UUID id, String token) {
